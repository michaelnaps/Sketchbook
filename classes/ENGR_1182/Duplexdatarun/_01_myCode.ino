void myCode()
{
  ...
  //start all motors at 45% power and continue for 132 marks
  motorSpeed(4,45);
  goToAbsolutePosition(132); 
  //airbrake at 40% for 1.7 seconds
  reverse(4);
  motorSpeed(4,40);
  goFor(1.7);
  //pause at stop for 3 seconds with hook displacement of 112 degrees
  brake(4);
  goFor(1);
  rotateServo(112);
  brake(4);
  ...
  //set motor speed to 25% power until the AEV reaches the origin point, then stop motors for 1 second
  motorSpeed(4,25);
  goToAbsolutePosition(0);
  brake(4);
  goFor(1);
  
  //set motor speed to 25% power until the AEV reaches 310 marks past the origin and then brake
  motorSpeed(4,25);
